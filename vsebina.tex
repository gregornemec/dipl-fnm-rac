...
primerov. Zanimal nas bo spletni portal, ki je nastal na \emph{odprti
  ...
  University of Technology, Australia} (\textbf {QUTA}). %Dodaj še druge UNI!
...
\section{Metode dela pri pouku računalništva}
...
\section{Organizacijske oblike pouka}
...
\subsection{Samostojno delo}
\label{sec:samostojn_delo}

Prvi način je morda najenostavnejši za organizacijo dela v učilnici in
omogoča aktivno učenje za vse učence. Taka oblika organizacije je
primerna predvsem, ko učitelj želi preveriti ali vsi učenci sledijo in
znajo uporabljati določeno znanje in veščine, kot je na primer uporaba
integriranega razvojnega okolja (\emph{ang. Integrated Development
  Environment +(\textbf{IDE})}) ali sledenje določenemu algoritmu.

\subsection{Delo v parih}
\label{sec:delo_v_parih}

Razred razdelimo v pare, ti rešujejo programerske ali ne programerske
naloge. V primeru programerskih nalog, učenca, ki sta v paru rešujeta
programersko nalogo tako da je eden v vlogi \textbf{voznika} in drugi
v vlogi \textbf{navigatorja}. Prvi, voznik ima v nadzoru tipkovnico in
...
proces reševanja problemov na dveh nivojih, en nivo predstavlja nalogo
kodiranja, drugi predstavlja uporabo strategij pri reševanju
...
možnost, da nekateri učenci dominirajo v skupini in teko druge učence
prikrajšajo za sodelovanje.

\subsection{Skupinsko delo}
\label{sec:skupinsko_delo}

Druga oblika organizacije je delo v skupi ali timu in je primerna v
naslednjih primerih:
...
\subsection{Skupinsko delo - sestavljanka (\emph{ang. Jigsaw
    Classroom})}
\label{sec:skupinsko_delo-sestavljanka}

Po navodilih spletne strani \href{https://www.jigsaw.org/}{razredne
  sestavljanke (\emph{ang. Jigsaw classroom})} je oblika organizacije
na naslednji način.
\begin{enumerate}
\tightlist
...
zaključi tudi brez končnega izdelka. Kot je razvidno z organizacije
dela \textbf{sestavljanke} so prednosti ogromne, tiste ki omogočajo
kognitivni razvoj in tiste, ki socialnega. Učenje v tej obliki
vzpodbuja učenje, poslušanje, sodelovanje in deljenje znanja.

\section{Splet in spletne tehnologije}
\label{sec:splet_in_spletne_tehnologije}
